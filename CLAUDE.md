...
- **CPU (RapidOCR)**：纯 CPU，无需额外环境。import 兼容 `rapidocr` 和 `rapidocr_onnxruntime` 两种包名
...
- **PyInstaller 打包兼容**：`PaddleOCREngine.__init__` monkey-patch 跳过 `require_extra` 和 `require_deps` 检查（PyInstaller 打包后元数据丢失 + opencv 包名冲突都会误报）
- **PyInstaller 窗口模式兼容**：`console=False` 时 `sys.stdout`/`sys.stderr` 为 `None`，文件顶部补 `io.StringIO()` 空流，防止 PaddleX 模型下载进度打印崩溃（这是"模型下载失败"的根因）
