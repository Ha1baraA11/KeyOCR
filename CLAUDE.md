...
多帧采样取中位数，避免单帧偏差导致检测失败：
1. 从粗扫帧目录优先密集采样前 25% 的帧（8帧）
2. 对每帧调用 detect_center_region 检测纯色区域
3. 过滤占比 20%-40% 的有效结果，取中位数
4. 前段帧不够时回退到全范围采样

单帧检测（detect_center_region）：局部标准差 → Otsu 阈值 → 形态学清理 → 最大轮廓 → 逐行方差裁剪

### OCR 模式切换
GUI 设置中支持两种 OCR 区域模式：
- **自动 OCR**：粗扫完成后自动调用 detect_stable_region 多帧采样检测
...
## OCR 区域检测测试结果
- 测试1.mp4 (544x960): y=339, h=289, 30.1%
- 测试2.mp4 (480x854): y=299, h=256, 30.0%
- 测试3.mp4 (480x854): y=299, h=256, 30.0%
- 测试4.mp4 (480x854): y=299, h=257, 30.1%
