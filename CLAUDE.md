...
- spec 用 `collect_binaries` + `collect_data_files` 收集原生二进制和数据文件
- CI 显式安装 paddleocr 全部传递依赖（pandas、scipy、shapely、pyclipper、requests 等）
...
- **GPU (PaddleOCR)**：需 CUDA 11.8 + cuDNN + paddlepaddle-gpu + pandas（PaddleX 依赖），失败时展示完整错误日志
