...
- CI 和 build.bat 打包后都运行自检（`ZHENTIQU_SELF_CHECK=1`），验证 EXE 内模块完整性 + CUDA 可用
...
- **自检机制**：`run_self_check()` 验证打包后 EXE 的依赖完整性，Windows 上还检查 `paddle_cuda=True`，通过 `ZHENTIQU_SELF_CHECK=1` 环境变量触发
