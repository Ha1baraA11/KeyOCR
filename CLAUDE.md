...
- 依赖：PySide6, opencv-contrib-python (4.10.0), numpy
- OCR：macOS 用 RapidOCR (CPU)，Windows 用 PaddleOCR (GPU 需 CUDA + paddlepaddle-gpu + paddlex[ocr])
...
python -m pip install pypdfium2 paddlepaddle-gpu paddleocr "paddlex[ocr]" rapidocr-onnxruntime
```
