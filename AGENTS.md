# AGENTS.md - ZhenTiqu (帧提取工具)

## 快捷指令
- **`update`** — 用户说 "update" 时，自动执行：1) 更新 memory；2) 更新 AGENTS.md；3) `git add . && git commit && git push` 推送到 GitHub。一次性完成，无需额外确认。
...
- **GitHub Actions CI 与 `build.bat` 都必须强制安装 `paddlepaddle-gpu==3.3.0` 并验证 `paddle.device.is_compiled_with_cuda()` 为 `True`**；绝不能在 GPU 安装失败后回退到 CPU 版继续打包，否则会发出“假 GPU release”
- 打包完成后必须运行 EXE 自检：Windows release 需要同时满足 `paddle_cuda=true` 与 `rapidocr_engine.ok=true`
- `frame_extractor.iss` — Inno Setup 安装脚本，默认装到 `D:\ZhenTiqu`（英文路径，避免中文路径和 Program Files 权限问题），支持自定义目录、桌面快捷方式
...
- `.Codex/task.md` — 任务状态记录
- `.Codex/test_results.md` — 测试结果记录
...
- **GPU (PaddleOCR 3.x)**：需 CUDA + paddlepaddle-gpu + `pip install "paddlex[ocr]"`，失败时展示完整错误日志
...
12. **PaddleX 文本检测阶段 `pyclipper` 未定义**
...
- 正确答案（37帧）：frame_000135 到 frame_004017，详见 `.Codex/task.md`
